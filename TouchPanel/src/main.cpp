...
#define TFT_BL 2
#define ambientLightCheckInterval 1000
...
/*void updateGPS(){
  lv_label_set_text_fmt(ui_Label2, "%d", int(trunc(gps.altitude.feet())));
}*/
...
void setup()
{
  ...
  longTermBrightness = map(ambientLight, 0,11,0,255);
...
void loop()
{
  ...
  if(millis() - ambientLightCheckStart >= ambientLightCheckInterval){
    ...
    if(map(ambientLight, 0,11,0,255) < longTermBrightness) longTermBrightness--;
    else if(map(ambientLight, 0,11,0,255) > longTermBrightness) longTermBrightness++;
